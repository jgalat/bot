module Main where
  ...
  compileFile file = do content <- catch (readFile file) (\e -> let err = show (e :: IOException)
                                                                in do putStrLn $ file ++ ": The file couldn't be opened.\n" ++ err
                                                                      return "")
                        case content of
                          ""  ->  return ("", Comm [] [])
  ...
  getConfiguration conf = do  content <- catch (readFile conf) (\e -> let err = show (e :: IOException)
                                                                      in do putStrLn $ conf ++ ": The file couldn't be opened.\n" ++ err
                                                                            return "")
                              case content of
